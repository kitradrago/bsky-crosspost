...
<body>
    <div class="login-page" id="loginPage">
        <div class="login-container">
            <div class="login-header">
                <h1>🚀 Bluesky Crosspost</h1>
                <p>Automatically share your Bluesky posts</p>
            </div>
    ...
    <div class="dashboard hidden" id="dashboard">
        <div class="navbar">
            ...
            <div class="navbar-right">
                <div id="restartBanner" class="restart-banner hidden">
                    ⚠️ Settings changed 
                    <button class="restart-btn" onclick="restartContainer()">Restart Now</button>
                </div>
        ...
        <div class="container">
            <div class="tabs">
                <button class="tab-btn active" onclick="showTab(event, 'setup')">⚙️ Setup</button>
                <button class="tab-btn" onclick="showTab(event, 'activity')">📊 Activity</button>
                <button class="tab-btn" onclick="showTab(event, 'logs')">📋 Logs</button>
                <button class="tab-btn" onclick="showTab(event, 'admin')">🔐 Admin</button>
            </div>
            <div class="page active" id="setupPage">
                <div class="card">
                    <div class="card-header"><h2>🔐 Bluesky Account</h2></div>
                    <div class="form-row">
                        <div class="form-field"><label>Bluesky Handle</label><input type="text" id="blueskyHandle" placeholder="example.bsky.social"></div>
                        <div class="form-field"><label>App Password</label><input type="password" id="blueskyPassword" placeholder="••••••••" autocomplete="new-password"></div>
                    </div>
                    <div class="form-row">
                        <div class="form-field"><label>Account to Monitor</label><input type="text" id="blueskyTargetHandle" placeholder="furthemore.org"></div>
                        <div class="form-field"><label>Check Interval (sec)</label><input type="number" id="blueskyCheckInterval" placeholder="300" min="10"></div>
                    </div>
                </div>
                <div class="card">
                    <div class="card-header"><h2>💬 Telegram</h2><label class="toggle-switch"><input type="checkbox" id="telegramEnabled"><span class="toggle-slider"></span></label></div>
                    <div id="telegramSettings"><div class="form-row"><div class="form-field"><label>Bot Token</label><input type="password" id="telegramBotToken" placeholder="123456:ABC-DEF..." autocomplete="new-password"></div><div class="form-field"><label>Channel ID</label><input type="text" id="telegramChannelId" placeholder="-1001234567890"></div></div></div>
                </div>
                <div class="card">
                    <div class="card-header"><h2>🎮 Discord</h2><label class="toggle-switch"><input type="checkbox" id="discordEnabled"><span class="toggle-slider"></span></label></div>
                    <div id="discordSettings"><div class="form-row"><div class="form-field"><label>Bot Token</label><input type="password" id="discordBotToken" placeholder="MTA4NzQ1..." autocomplete="new-password"></div><div class="form-field"><label>Channel ID</label><input type="text" id="discordChannelId" placeholder="1087459487405..."></div></div></div>
                </div>
                <div class="card">
                    <div class="card-header"><h2>🐾 FurAffinity</h2><label class="toggle-switch"><input type="checkbox" id="furAffinityEnabled"><span class="toggle-slider"></span></label></div>
                    <div id="furAffinitySettings"><div class="form-row"><div class="form-field"><label>Username</label><input type="text" id="furAffinityUsername" placeholder="your_username"></div><div class="form-field"><label>Password</label><input type="password" id="furAffinityPassword" placeholder="••••••••" autocomplete="new-password"></div></div><div class="form-row"><div class="form-field"><label>Image Category</label><select id="furAffinitySubmissionCategory"><option value="1">Artwork/Digital</option><option value="2">Photography</option><option value="3">Traditional Art</option><option value="4">Sculpture</option><option value="5">Other</option></select><div class="helper-text">Used for image submissions only</div></div></div><div class="form-row"><div class="form-field"><label>Rating</label><select id="furAffinitySubmissionRating"><option value="general">General</option><option value="mature">Mature</option><option value="adult">Adult</option></select></div><div class="form-field"><label><input type="checkbox" id="furAffinityDownloadImages"> Download & Submit Images</label><div class="helper-text">Auto-detect images in posts and submit as images when available, otherwise post as journal</div></div></div></div>
                </div>
                <button class="btn btn-primary" onclick="saveConfig()">💾 Save Settings</button>
                <div id="setupAlert"></div>
            </div>
            <div class="page" id="activityPage">
                <div class="card"><h2>📊 Activity Overview</h2><div class="stats-grid"><div class="stat-card"><h3>Posts Processed</h3><div class="number" id="totalPosts">-</div></div><div class="stat-card"><h3>Successful</h3><div class="number" id="successfulPosts">-</div></div><div class="stat-card"><h3>Telegram</h3><div class="number" id="telegramCount">-</div></div><div class="stat-card"><h3>Discord</h3><div class="number" id="discordCount">-</div></div><div class="stat-card"><h3>FurAffinity</h3><div class="number" id="furAffinityCount">-</div></div></div></div>
                <div class="card"><h2>📝 Recent Posts</h2><table class="posts-table"><thead><tr><th>Date & Time</th><th>Post</th><th>Status</th><th>Telegram</th><th>Discord</th><th>FurAffinity</th><th>Actions</th></tr></thead><tbody id="postsTableBody"><tr><td colspan="7" style="text-align: center; padding: 40px;">Loading...</td></tr></tbody></table></div>
            </div>
            <div class="page" id="logsPage">
                <div class="card"><h2>📋 System Logs</h2><div class="log-controls"><div class="form-field" style="margin: 0; flex: 1; min-width: 150px;"><label>Show last:</label><select id="logLines" onchange="loadLogs()" style="margin-top: 4px;"><option value="50">50 lines</option><option value="100" selected>100 lines</option><option value="200">200 lines</option><option value="500">500 lines</option></select></div><button class="btn btn-secondary" onclick="loadLogs()" style="margin-top: 22px;">🔄 Refresh Logs</button><label style="margin-top: 22px;"><input type="checkbox" id="autoRefreshLogs"> Auto-refresh</label></div><div class="logs-container" id="logsContainer"><div class="log-line">Loading...</div></div></div>
            </div>
            <div class="page" id="adminPage">
                <div class="card"><h2>🔐 Admin Settings</h2><div class="form-row"><div class="form-field"><label>Admin Username</label><input type="text" id="adminUsername" placeholder="admin" autocomplete="off"><div class="helper-text">Username for web interface</div></div><div class="form-field"><label>Admin Password</label><input type="password" id="adminPassword" placeholder="••••••••" autocomplete="new-password"><div class="helper-text">Password for web interface (will be encrypted)</div></div></div><button class="btn btn-primary" onclick="saveAdminSettings()">💾 Save Admin Settings</button><div id="adminAlert"></div></div>
            </div>
        </div>
    </div>
    <script>
