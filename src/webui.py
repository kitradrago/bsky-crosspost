...
class WebUI:
    ...
    async def handle_login(self, request):
        ...
        try:
            ...
            # Try decrypting the stored password
    ...
    async def get_logs(self, request):
        ...
        if not self._is_authenticated(request):
            logger.warning(f"❌ GET /api/logs - Auth failed")
            return web.json_response({'error': 'Unauthorized'}, status=401)
        ...
        try:
            ...
            logger.info(f"📋 GET /api/logs - Auth passed, reading {lines} lines from {log_file}")
    ...
    def _ensure_certificate(self):
        """Ensure we have a valid certificate. Generate if needed."""
...
<body>
    ...
    <script>
        ...
        window.onload = async function() {
            ...
            if (authToken) {
                try {
                    console.log('Checking existing token from localStorage: ' + authToken.substring(0, 8) + '...');
        ...
        async function handleLogin(e) {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    console.log('✅ Login successful! Token:', authToken.substring(0, 8) + '...');
                    showDashboard();
                    await loadConfig();
                    await loadPosts();
                    await loadLogs();
        ...
        async function apiCall(url, method, body) {
            method = method || 'GET';
            const opts = {method: method, headers: {'Authorization': 'Bearer ' + authToken}};
            if (body) {
        ...
        async function refreshData() {
            ...
            if (tab && tab.textContent.includes('Activity')) {
                await loadPosts();
            } else if (tab && tab.textContent.includes('Logs')) {
                await loadLogs();
            }
        }
    </script>
