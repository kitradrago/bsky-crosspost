...
class CrosspostManager:
    ...
    async def process_new_posts(self):
        ...
        try:
            # Check last 24 hours for new posts
            posts = await self.bluesky.get_recent_posts(limit=50, hours_back=24)
...
async def main():
    ...
    # Set cross-post callback for WebUI retries and manual checks
    async def retry_callback(post_uri: str = None, services: list = None, hours_back: int = None) -> dict:
